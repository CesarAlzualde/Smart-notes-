...
def create_complete_filter_map():
    ...
    try:
        ...
        with driver.session() as session:
            # Primero, eliminar cualquier nodo o relación existente para limpiar
            session.run("""
            MATCH (m:ConceptMap {id: 'filter'})
            OPTIONAL MATCH (m)-[r1:BASED_ON]->(n:NoteReference)
            OPTIONAL MATCH (c:Concept {map_id: 'filter'})
            OPTIONAL MATCH (c1:Concept {map_id: 'filter'})-[r2:RELATES_TO]->(c2:Concept {map_id: 'filter'})
            DELETE r1, r2, n, c, m
            """)
            
            logger.info("Limpiado cualquier mapa existente con ID 'filter'")
            
            # Crear el nodo ConceptMap con todas las propiedades requeridas
            result = session.run("""
            CREATE (m:ConceptMap {
                id: 'filter', 
            ...
            # Mostrar las propiedades del nodo ConceptMap
            result = session.run("""
            MATCH (m:ConceptMap {id: 'filter'})
...
if __name__ == "__main__":
    ...
    print(f"Script completado {'con éxito' if success else 'con errores'}.\n")
